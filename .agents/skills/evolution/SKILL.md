...
For reliable automatic triggering, use Codex hooks. Install with `--with-hooks`:
...
This will install hooks to `.Codex/hooks/` and configure `.Codex/settings.json`:
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .Codex/hooks/makepad-skill-router.sh"
          }
        ]
      }
    ],
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .Codex/hooks/pre-tool.sh"
          }
        ]
      }
    ],
...
```
User follows skill advice → Code fails to compile/run → Codex identifies skill was wrong
                                                      ↓
...
Codex should detect Makepad version from:
...
Codex analyzes the current project to detect:
