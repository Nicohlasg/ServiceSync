...
description: "Diagnose and optimize Agent Skills (SKILL.md) with real session data and research-backed static analysis. Works with Codex, Codex, and any Agent Skills-compatible agent."
risk: safe
...
CSO (Codex/Agent Search Optimization) = writing skill descriptions so agents select the right skill at the right time. This skill checks for CSO violations.
...
| Source | Codex | Codex | Shared |
|--------|------------|-------|--------|
| Session transcripts | `~/.Codex/projects/**/*.jsonl` | `~/.codex/sessions/**/*.jsonl` | — |
| Skill files | `~/.Codex/skills/*/SKILL.md` | `~/.codex/skills/*/SKILL.md` | `~/.agents/skills/*/SKILL.md` |

**Platform detection:** Check which directories exist. Scan all available sources — a user may have both Codex and Codex installed.
...
Scan skill directories in order: `~/.Codex/skills/`, `~/.codex/skills/`, `~/.agents/skills/`. Deduplicate by skill name (same name in multiple locations = same skill). For each, read `SKILL.md` and extract:
...
**Codex sessions** (`~/.Codex/projects/**/*.jsonl`):
...
**Codex:** count `Skill` tool_use calls in transcripts.
