...
- `.env` file with `APIFY_TOKEN` (at `~/.Codex/.env`)
...
```bash
SKILL_PATH=~/.Codex/skills/apify-ecommerce
```
...
```bash
node --env-file=~/.Codex/.env $SKILL_PATH/reference/scripts/run_actor.js \
...
| Error | Solution |
|-------|----------|
| `APIFY_TOKEN not found` | Ensure `~/.Codex/.env` contains `APIFY_TOKEN=your_token` |
