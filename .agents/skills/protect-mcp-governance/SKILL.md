...
3. **Hooks mode** — integrates with Codex hooks for pre/post tool-call governance.
...
```bash
# Install and initialize hooks (Codex integration)
...
### Example 1: Governance for a Codex Session

```bash
# Initialize hooks
npx protect-mcp init-hooks

# Codex now generates a signed receipt for every tool call.
# Receipts are stored in .protect-mcp/receipts/
```

**Explanation:** After initialization, every tool call Codex makes is logged with a signed receipt. No tool calls are blocked (shadow mode).
