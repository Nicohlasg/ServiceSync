...
3. **Add the MCP server:**
   - Codex: `Codex mcp add jobgpt -t http -u https://mcp.6figr.com/mcp --header "Authorization: <api-key>"`
...
| Problem | Solution |
|---------|----------|
| "Missing Authorization header" | For Codex and other remote HTTP MCP setups, confirm the `Authorization` header is configured on the MCP server entry |
