...
Use Python or bash with AI model (Codex/GPT) for intelligent summarization:
...
def generate_summary(segments, max_paragraphs=5):
    """Create executive summary using AI (Codex/GPT via API or local model)."""
    ...
    # Call AI model (placeholder - user can integrate Codex API or use local model)
...
def process_with_llm(transcript, prompt, cli_tool='Codex'):
