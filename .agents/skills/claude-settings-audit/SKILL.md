---
name: Codex-settings-audit
description: Analyze a repository to generate recommended Codex settings.json permissions. Use when setting up a new project, auditing existing settings, or determining which read-only bash commands to allow. Detects tech stack, build tools, and monorepo structure.
risk: unknown
source: community
---

# Codex Settings Audit

Analyze this repository and generate recommended Codex `settings.json` permissions for read-only commands.

## When to Use

- You are setting up or auditing Codex `settings.json` permissions for a repository.
- You need to infer a safe read-only allow list from the repo's tech stack, tooling, and monorepo structure.
- You want to review or replace an existing Codex permissions baseline with something evidence-based.
...
```bash
cat .Codex/settings.json 2>/dev/null || echo "No existing settings"
```
...
[
  ...
  "Skill(sentry-skills:Codex-settings-audit)",
...
## Recommended .Codex/settings.json
