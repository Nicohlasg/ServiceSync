...
2. Choose the provider-appropriate method:
   ...
   - **Anthropic (Codex):** Define a single tool with the target schema as `input_schema` and set `tool_choice: { type: "tool", name: "extract_data" }`. Codex returns the structured data in the `tool_use` content block.
