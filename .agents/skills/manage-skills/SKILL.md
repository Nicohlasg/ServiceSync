---
name: manage-skills
description: Discover, list, create, edit, toggle, copy, move, and delete AI agent skills across 11 tools (Cursor, Codex, Agents, Windsurf, Copilot, Codex, Cline, Aider, Continue, Roo Code, Augment)
---
...
For directory-based tools (Agents, Cursor, Codex), skills use YAML frontmatter:
...
echo "Codex: $(ls ~/.Codex/skills/ 2>/dev/null | wc -l | tr -d ' ')"
...
```bash
# For Agents/Cursor/Codex (SKILL.md format)
...
```bash
# Copy from Cursor to Codex
cp -r ~/.cursor/skills/my-skill ~/.Codex/skills/my-skill
...
```bash
# Search by name
find ~/.agents/skills ~/.cursor/skills ~/.Codex/skills ~/.windsurf/rules ~/.cline/rules ~/.continue/rules ~/.roo/rules -maxdepth 1 -type d 2>/dev/null | sort

# Search by content
grep -rl "search term" ~/.agents/skills/ ~/.cursor/skills/ ~/.Codex/skills/ 2>/dev/null
```

### Find disabled skills

```bash
find ~/.agents/skills ~/.cursor/skills ~/.Codex/skills -name "*.disabled" 2>/dev/null
```
