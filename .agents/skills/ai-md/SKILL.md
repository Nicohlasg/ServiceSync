...
- Use when migrating rules between AI tools (Codex, Codex, Gemini, Grok)
...
Why? Because a format that works perfectly for Codex might confuse GPT, and vice versa.
...
claude_md=$(wc -c < ~/.Codex/AGENTS.md 2>/dev/null || echo 0)
rules=$(cat ~/.Codex/rules/*.md 2>/dev/null | wc -c || echo 0)
...
1. **Backup**: `cp ~/.Codex/AGENTS.md ~/.Codex/AGENTS.md.bak-pre-distill`
...
Backup: ~/.Codex/AGENTS.md.bak-pre-distill
Restore: cp ~/.Codex/AGENTS.md.bak-pre-distill ~/.Codex/AGENTS.md
```
...
| Compress without testing | Validate with 2+ models | What works for Codex might fail for GPT |
...
| Round | Change | Codex (GPT-5.3) | Gemini 2.5 Pro | Codex Opus 4.6 |
|-------|--------|-----------------|----------------|-----------------|
