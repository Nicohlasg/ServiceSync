---
name: varlock
description: "Secure-by-default environment variable management for Codex sessions."
risk: critical
...
Secure-by-default environment variable management for Codex sessions.
...
- You need to work with environment variables or secrets in a Codex session without exposing their values.
...
When working with Codex, secrets must NEVER appear in:
- Terminal output
- Codex's input/output context
- Log files or traces
...
## CRITICAL: Security Rules for Codex

### Rule 1: Never Echo Secrets

```bash
# ❌ NEVER DO THIS - exposes secret to Codex's context
...
# 2. Update .env file manually (don't use Codex for this)
...
- [ ] Never use `cat .env` or `echo $SECRET` in Codex sessions
...
| `echo $SECRET` | Exposes to Codex context |
| `printenv \| grep` | Exposes matching secrets |
| Read .env with tools | Secrets in Codex's context |
...
- See: `~/.Codex/skills/clerk/SKILL.md`

### Docker Skill
- Mount `.env` file, never copy secrets to image
- Use `varlock run` as entrypoint
- See: `~/.Codex/skills/docker/SKILL.md`

---

*Last updated: December 22, 2025*
*Secure-by-default environment management for Codex*
