@agent-orchestrator @trpc-fullstack @backend-security-coder @frontend-developer @database-admin

We are executing a targeted Pre-Launch Audit Remediation for "ServiceSync V2". I have an `audit-tracker.md` file in the project root containing all Critical and High issues.

I want you to act as an autonomous agent orchestrator to systematically fix these issues one by one, pausing after each fix so we maintain absolute stability. 

Here is your workflow:
1. **Read `audit-tracker.md`**: Find the FIRST unchecked `- [ ]` task starting from Phase 1.
2. **Analyze**: Use `@backend-security-coder` (for Phase 1 security/CORS/auth issues) or `@trpc-fullstack` / `@frontend-developer` to locate the files mentioned in the task and understand the current implementation.
3. **Remediate**: Write the necessary code to fix the issue. Use `@database-admin` for Supabase RLS policies, database changes, and schema drift fixes. Remove hardcoded mocks, fix imports, and properly wire up tRPC routers or environment variables as required by the task.
4. **Verify**: Briefly explain how the code changes address the core vulnerability or functional gap.
5. **Update Tracker**: Mark the checkbox as `[x]` in `audit-tracker.md` for the task you just completed.
6. **Pause**: Stop executing. Ask me to review the changes and run `git commit` before you proceed to the next unchecked item.

Start right now by reading `audit-tracker.md`, identifying the very first uncompleted task, and beginning your analysis and code changes.---
